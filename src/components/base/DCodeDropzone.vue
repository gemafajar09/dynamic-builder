...
function onDrop(e: DragEvent) {
  ...
  const filtered = props.accept?.includes("image/")
...
const acceptPretty = computed(() =>
  (props.accept || "file/*")
    ...
    .filter(Boolean)
    .map((s) =>
      s === "image/*" ? "IMAGE" : s.replace(/^image\//, "").replace(/^\./, ""),
    )
    .join(", "),
);
...
<template>
  <Card :class="disabled ? 'opacity-60' : ''">
    ...
    <CardContent class="pt-2">
      <div
        class="flex justify-center items-center rounded-xl border-2 border-dashed transition cursor-pointer select-none"
        :class="[
          isDragging
            ? 'border-primary bg-primary/10 -translate-y-[1px]'
      ...
      >
        <div
          class="h-full w-full p-6 flex flex-col items-center justify-center text-center gap-2"
        >
          ...
          <Badge
            v-if="showSelected && lastFileName"
            variant="secondary"
            class="mt-2"
          >
            {{ lastFileName }} •
            {{ lastFileSize ? humanSize(lastFileSize) : "" }}
          </Badge>
        </div>

...
function syncCommittedFromModel() {
  ...
  committedText.value = committedRange.value
    ? rangeText(committedRange.value)
    : "";
}
...
function applyDraftTextToDraftRange(): boolean {
  ...
  if (a && !startDt) {
    emit(
      "invalid",
      "Format tanggal start tidak valid. Pakai YYYY-MM-DD atau DD/MM/YYYY.",
    );
    return false;
  }
  ...
  if (b && !endDt) {
    emit(
      "invalid",
      "Format tanggal end tidak valid. Pakai YYYY-MM-DD atau DD/MM/YYYY.",
    );
    return false;
  }
    ...
    <PopoverTrigger as-child>
      <div class="relative w-full">
        <Input
          ...
          @keydown.enter.prevent="applyDraftTextToDraftRange()"
          @blur="applyDraftTextToDraftRange()"
        />

        <Button
          v-if="clearable && draftText"
          type="button"
          variant="ghost"
          size="icon"
          class="absolute right-10 top-1/2 -translate-y-1/2 h-8 w-8"
          :disabled="disabled"
          @click="onClear"
        >
          ✕

<template>
  <div class="d-datatable-container">
    <div
      v-if="config.titleID"
      class="d-title mb-4 flex justify-between items-center"
    >
    ...
    <div
      v-if="config.filter && config.filter.length"
    ...
    <table class="w-full border-collapse border">
      ...
      <tbody>
        ...
        >
          ...
          <td v-if="hasRowActions" class="border p-2">
            <div class="flex gap-2">
              <button
                @click.stop="handleRowAction('edit', row)"
                class="text-blue-500 underline"
              >
                Edit
              </button>
              <button
                @click.stop="handleRowAction('delete', row)"
        ...
        <tr v-if="data.length === 0 && !loading">
          <td
            :colspan="config.headers.length + (hasRowActions ? 1 : 0)"
